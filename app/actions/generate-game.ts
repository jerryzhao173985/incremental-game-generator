...
import { chatCompletionWithFallback, AVAILABLE_MODELS } from "@/lib/openai-utils"
...
Focus on making the game fun and engaging for players.`
    ...
    const data = await chatCompletionWithFallback({
      apiKey,
      model,
      messages: [
        {
...
export async function generateGameStage(
  ...
  try {
    ...
    const stageSpec = await generateStageSpec(stageNumber, theme, previousStages, apiKey, model)
...
Ensure the HTML, CSS, and JavaScript work together properly and the game is functional in all contexts.`
    ...
    try {
      // Call OpenAI API to generate game code with fallback
      const data = await chatCompletionWithFallback({
        apiKey,
        model,
        messages: [
...
Ensure the game initializes properly with DOMContentLoaded.`
    ...
    // Call OpenAI API to fix the game code with fallback
    const data = await chatCompletionWithFallback({
      apiKey,
      model,
      messages: [
